...
## Runtime Notes
- Durable worker tool parity (`apps/api/src/agent_tools.rs`, `agent_runner.rs`): the cloud loop authorizes each call against the run's permission profile (network/shell/write/read/matrix, workspace roots confined to trusted roots), executes via `ToolExecutor` + Postgres memory collections + `PluginManager` (connectors/MCP/skills), persists steps/artifacts/context, feeds truncated results back to history, times out calls (90 s) and fails the job after 3 consecutive tool errors. Desktop-only capabilities fail closed with `worker_unsupported_tool`, never a fake success.
- Web SSE parsing (`lib/sse.ts`): framing state (buffer + event type) survives TCP chunk splits; blank-line dispatch, comments ignored.
- Cloud sync (`lib/api/cloud-sync.ts`): durable identity-scoped queue (localStorage-backed, survives reloads), never drops on failure (capped exponential backoff + 429 circuit breaker), per-key isolation so one poison entry never blocks others, `setIdentityScope`/`reset` wired to login/logout/org-switch.
- Device cache hygiene (`src-tauri/state.rs`): `cloud_identity.json` sidecar tracks the owning cloud user; a different user logging in wipes the local SQLite AI cache via `reset()`. Same-user login keeps everything.
- Tenant isolation is enforced by Postgres RLS on conversations/messages/memories (`202607290001_private_data_rls_enforce.sql`): every request-scoped query runs in a tenant transaction (`begin_tenant_tx`), worker paths set per-unit-of-work context, and the multi-tenant `reconcile` sweep evaluates the gated leg per job. Proven by `tests/rls.rs` (app layer) and `tests/rls_enforcement.rs` (least-privilege role: no context → nothing, foreign tenant → nothing, own rows visible, cross-tenant writes rejected).
- Skill scripts run sandboxed server-side (`aro-skills/sandbox.rs`: staged copy with size cap, secret env scrubbed, timeout kill, output caps, concurrent pipe drains), gated by `allow_shell` in the worker matrix.
- API handler layout: `handlers.rs` keeps routes + core domains; `agent_tools.rs` (tool execution), `handlers_plugins.rs` (plugins/MCP/skills), `handlers_notify.rs` (notifications/email) are separate modules re-exported through `handlers::`.
- Worker memory writes also reindex vectors best-effort (`WorkerToolDeps.vector`; FTS stays authoritative, failures only warn).
- Desktop model-provider domain lives in `features/model-providers/` (pure catalog builders in `modelProviders.ts`, async orchestrators in `modelOrchestrators.ts` behind an explicit context); `App.svelte` only wires state and template.
- Agent model/tool loop runs until the model returns final/pause (or a validation/provider error), guarded by an independent safety ceiling: `AgentLoopBudget` (default 200 model iterations / 600 s wall-clock, via `ARO_AGENT_MAX_STEPS` / `ARO_AGENT_MAX_WALL_SECS`, `0` disables an axis). Explicit per-request `max_steps` tightens the ceiling further. The old 8-default/32-hard-cap only survives as legacy constants in old migrations; current job-budget ceiling is 32767 (`202607260001_agent_unlimited_steps.sql`).
